...
func (self *Route) Match(path string) (map[string]string){
  if (self.matcher == nil) {
    self.matcher = regexp.MustCompile(self.Regexp)
  }
  args := self.matcher.FindAllStringSubmatch(path, -1)
  if args != nil {
    // dumpValue(args)
...
func (self * Route) Complie (params map[string]interface{}) (string) {
  ...
  return self.ComplieStrings(datas)
}

func (self *Route) ComplieStrings(params map[string]string) (path string){
